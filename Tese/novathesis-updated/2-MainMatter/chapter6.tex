...
didn't pertain to me. Furthermore, deciding algorithmically which output-carrying $\varepsilon$-transition FSTs 
can be successfully determinized introduces computational complications, as it requires tracking unbounded lookaheads and resolving complex 
prefix-consistency guarantees across epsilon cycles. So the decision to restrict determinization to only handling transducers whose $\varepsilon$-transitions carry no output,
...
\texttt{toDeterministic}, handles transducers whose $\varepsilon$-transitions carry no output. 
