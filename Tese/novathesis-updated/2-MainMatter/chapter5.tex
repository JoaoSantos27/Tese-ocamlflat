...
\section{Why OCaml for OFLAT}
OCaml offers strong static typing, pattern matching, and algebraic data types that map naturally to formal-language structures 
(symbols, words, transitions, grammars). Immutable defaults and exhaustive matches help prevent logic gaps in algorithms 
such as determinization, minimization, and composition of FSTs. The module system keeps model-specific code isolated yet composable, 
which suits the multi-model nature of OFLAT (FA, RE, CFG, PDA, TM, FST).
...
\subsection{Bindings}
Bindings are a crucial component for enabling interoperability between OCaml and JavaScript. 
...
The following OCaml type, as shown below, describes the structure of the Cytoscape objects, and it allows method invocation with type safety:
