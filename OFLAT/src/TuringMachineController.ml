...
class tmController (tm: TuringMachineView.model) (s: bool)=
  ...
  method defineExample =
    ...
    myTM#drawExample self#getCy (Settings.getSetting "layout");
    self#defineInformationBox;
    Cytoscape.fit self#getCy_opt

  method redrawLayout =
    self#operationTM "redraw layout";
    self#updateButtons;
    HtmlPageClient.putCyTMButtons ();
    myTM#redrawLayout self#getCy (Dom_html.getElementById "cy");
    self#defineInformationBox;
    Cytoscape.fit self#getCy_opt
  ...
  method defineInformationBox =
    let name = myTM#getName in
    let isDeter = myTM#isDeterministic in 
    let hasUseless = not myTM#areAllStatesUseful in
    let nUseless = myTM#getUselessStates in 
    let nStates = myTM#numberStates in 
    let nTrans = myTM#numberTransitions in
    let isLB = myTM#isLB in
      HtmlPageClient.drawAutomatonStats (Lang.i18nTM ()) name isDeter hasUseless nUseless nStates nTrans isLB side
  ...
  *)
  ...
  method private resetAndRedraw =
    ...
    myTM#drawExample cy (Settings.getSetting "layout");
    self#defineInformationBox
...
*)      
      ...
      ( 
        if (not (myTM#hasState node)) then 
          ...
          (
            ...
            Set.iter (fun (a,b,c,d,e) -> eliminateNodeTransitions (a, myTM#makeLabel b d e, c) node)
  ...
  method addInitialNode: unit =
      self#operationTM "Add Initial Node";
      let result = self#checkForSimulation in
      if result then 
        (          
          let promptResult = (JS.prompt (Lang.i18nTextEnterState ()) "A") in
    ...
    method turnInitialNode node =
      self#operationTM "turn node initial";
      let result = self#checkForSimulation in
      if result then 
        ( 
          if (myTM#isInitial node) then
            ...
            (
              self#changeToEditMode;
              myTM <- myTM#turnNodeInitial node;
              Cytoscape.resetFaElems self#getCy;
              self#defineExample;
              self#defineInformationBox
            )
